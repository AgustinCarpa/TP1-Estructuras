...
(define (draw s)
  (place-image (draw-points s) (- WIDTH 10) 10
               (place-image BALL (st-ball-x s) (st-ball-y s)
                            (place-image BAR BAR-X (st-bar-y s) (empty-scene WIDTH HEIGHT)
                            )
               )
  )
)
...
(define (add-point s)
  (cond [(hit-bar? s) (make-st (st-ball-x s) (st-ball-y s) (st-bar-y s) (st-ball-vx s) (st-ball-vy s) (+ (st-points s) 1))]
        [else s]
  )
)
...
(define (bounce-x s)
  (cond
      [(hit-bar? s)        (reflect-ball-x s 6)]
      [(hit-right-wall? s) (reflect-ball-x s -6)]
      [else s]
  )
)
...
(define (move-bar s n)
   (cond [(and (>= (- (st-bar-y s) (/ BAR-HEIGHT 2) BAR-VEL) 0)
               (<= (+ (st-bar-y s) (/ BAR-HEIGHT 2) BAR-VEL) HEIGHT))
...
(define (stop? s)
  (or (= (st-points s) 20) (<= (- (st-ball-x s) BALL-RADIUS) 0))
)
